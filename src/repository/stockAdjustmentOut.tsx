...
const autoGenerateSuffix = 'ADJUSTMENTOUTNUMBER-MOBILE';
...
export async function incLatestRefNumber(branch: number) {
  ...
  incAutoGeneratedNumber(branch, 'ADJUSTMENTOUTNUMBER');
}
...
export async function saveStockAdjustmentOut(data: StockAdjustmentOut) {
  ...
  if (insertId && data.items) {
    data.items?.map((i: StockAdjustmentOutItem) => saveStockAdjustmentOutItem({...i, adj_id: insertId}))
  }
