...
const autoGenerateSuffix = 'TRANSFEROUTNUMBER-MOBILE';
...
export async function incLatestRefNumber(branch: number) {
  ...
  incAutoGeneratedNumber(branch, 'TRANSFEROUTNUMBER');
}
...
export async function saveStockTransferOut(data: StockTransferOut) {
  ...
  if (insertId && data?.items) {
    data.items?.map((i: StockTransferOutItem) => saveStockTransferOutItem({...i, ref_id: insertId}))
  }

  if (data.source_branch_id) await incLatestRefNumber(data.source_branch_id);
}
